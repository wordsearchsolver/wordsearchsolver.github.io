...
S H A R K W H A L E`,
  ...
  init() {
    ...
    // This solver UI only applies to the word-search tool page. Other tool
    // pages (crossword solver, etc.) share this same app.js for the header,
    // footer, FAQ, counters, and scroll animations, but don't have these
    // elements — so bail out here instead of throwing later in
    // updateGridInfo() and silently killing the rest of DOMContentLoaded.
    if (!this.gridInput || !this.wordsInput || !this.resultsSection) return;
  ...
  updateGridInfo() {
    ...
    if (grid.length > 0 && infoEl) {
      const rows = grid.length;
      const cols = Math.max(...grid.map(r => r.length));
      infoEl.textContent = `${rows} × ${cols} grid detected`;
    }
  },
